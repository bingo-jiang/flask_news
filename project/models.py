...
tb_user_follows = db.Table(
    "info_user_fans",
    db.Column('follower_id', db.Integer, db.ForeignKey('info_user.id'), primary_key=True),  # 粉丝id,即登录用户的user_id
    db.Column('followed_id', db.Integer, db.ForeignKey('info_user.id'), primary_key=True)  # 被关注人的id,即文章的author_id
)
...
class User(BaseModel, db.Model):
    ...
    avatar_url = db.Column(db.String(256), default="/static/images/default.jpg")  # 用户头像路径
...
class News(BaseModel, db.Model):
    ...
    status = db.Column(db.Integer, default=0)  # 当前新闻状态 如果为1代表审核通过，0代表审核中，-1代表审核不通过
    collected_counts = db.Column(db.Integer, default=0)  # 被收藏的计数
...
class Comment(BaseModel, db.Model):
    ...
    def to_dict(self):
        resp_dict = {
            ...
            "news": News.query.get(self.news_id).to_dict(),
...
class Category(BaseModel, db.Model):
    ...
    status = db.Column(db.Boolean, default=False, nullable=False)  # 是否使用
...
class AuthorRegister(BaseModel, db.Model):
    """作者注册信息"""
    __tablename__ = "author_register_info"
    id = db.Column(db.Integer, primary_key=True)  # 编号
    name = db.Column(db.String(256), nullable=False)  # 姓名
    id_card = db.Column(db.String(18), nullable=False)  # 身份证
    email = db.Column(db.String(32), nullable=False)  # 邮箱
    address = db.Column(db.String(128), nullable=False)  # 住址
    positive_ID_card_url = db.Column(db.String(256))  # 身份证正面图
    take_ID_card_url = db.Column(db.String(256))  # 手持身份证正面图
    status = db.Column(db.Integer, default=0)  # 当前审核状态 如果为1代表审核通过，0代表审核中，-1代表审核不通过
    reason = db.Column(db.String(256))  # 未通过原因，status = -1 的时候使用
    user_id = db.Column(db.Integer, db.ForeignKey("info_user.id"))  # 关联的用户ID
    ...
    def to_dict(self):
        resp_dict = {
            ...
            "positive_ID_card_url": self.positive_ID_card_url,
            "take_ID_card_url": self.take_ID_card_url,
            "status": self.status,
            "reason": self.reason,
        }
        return resp_dict
...
class Notice(BaseModel, db.Model):
    """公告"""
    ...
    admin_id = db.Column(db.Integer, db.ForeignKey("info_user.id"))  # 发布者
    ...
    def to_dict(self):
        resp_dict = {
            ...
            "title": self.title,
            "content": self.content,
            "admin": User.query.get(self.admin_id).to_dict(),
        }
        return resp_dict
